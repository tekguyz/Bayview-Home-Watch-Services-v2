...
export function ServicesSection() {
  ...
  useEffect(() => {
    ...
    observerRef.current = new IntersectionObserver(handleIntersect, {
      root: null,
      rootMargin: "-25% 0px -65% 0px", // Trigger when the section occupies the upper-middle region of viewport
      threshold: 0,
    });

    SERVICES.forEach((service) => {
      const el = document.getElementById(`category-${getCategorySlug(service.category)}`);
  ...
  const handleTabClick = (categoryName: string) => {
    ...
    if (element) {
      // Offset scrolling slightly to account for the sticky Nav
      const yOffset = -100;
  ...
  return (
    <SectionWrapper id="services" className="bg-white">
      <Container>
        ...
        {/* Category Navigation (Horizontal scrollable pill tabs) */}
        <div className="sticky top-[72px] z-30 -mx-[clamp(1.25rem,5vw,3rem)] px-[clamp(1.25rem,5vw,3rem)] py-3 bg-white/95 backdrop-blur-md border-b border-cream-dark mb-10">
          <div className="flex gap-2 overflow-x-auto scrollbar-hide pb-1 flex-nowrap select-none items-center justify-start md:justify-center">
            {SERVICES.map((cat) => {
              ...
              return (
                <button
                  key={cat.category}
                  onClick={() => {
                    setActiveTab(slug); // existing state update
                    const targetSlug = cat.category.toLowerCase().replace(/\s+/g, "-").replace(/[^a-z0-9-]/g, "");
                    document.getElementById(`category-${targetSlug}`)?.scrollIntoView({ behavior: "smooth", block: "start" });
                  }}
          ...
          {SERVICES.map((category) => {
            ...
            return (
              <div
                ...
                id={`category-${category.category.toLowerCase().replace(/\s+/g, "-").replace(/[^a-z0-9-]/g, "")}`}
              ...
              >
                ...
                {/* Grid of service cards inside this category */}
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5 mt-6">
                  {category.items.map((item, itemIndex) => {
                    // Injecting category icon as background watermarked fallback in card if needed, or just let card draw it
                    return (
                      <ServiceCard
                        key={itemIndex}
                        name={item.name}
